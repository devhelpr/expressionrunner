...
import {invalidOperatorAfterOperator, invalidCloseTag} from './expression-resource-strings';
...
export function ExpressionParser(expression : string) {
	...
	while (index < expression.length) { 
		...
		if (expressionState === ExpressionState.empty || 
			...
			if (character === ",") { 				
				if (currentNode.parentNode && currentNode.parentNode.nodeType === ExpressionNodeType.expression) {
			...
			if (character === ")") {
				...
				if (currentNode.parentNode && currentNode.parentNode.nodeType === ExpressionNodeType.expression) {
					...
					console.log(invalidCloseTag, currentNode, rootNode, expressionState, currentOperator, character);
		...
		if (expressionState === ExpressionState.operator) {
			if ((character >= '0' && character <= '9') || character === '.') {
			...
			if ("+-/*^".indexOf(character) >= 0) {
				console.error(invalidOperatorAfterOperator, rootNode, expressionState, currentOperator, character);
				throw new Error(invalidOperatorAfterOperator);
			}

		} else
		if (expressionState === ExpressionState.alpha) {
			if (
